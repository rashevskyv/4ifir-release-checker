...
async def download_file(bot, message_obj, file_name):
    ...
    try:
        ...
        try:
            ...
            # Відправляємо повідомлення про початок завантаження (тільки в лог, щоб не спамити користувачів)
            # Можна закоментувати відправку повідомлення, якщо надто багато спаму
            ...
            logger.warning(f"Не вдалося завантажити {file_name} через Bot API ({e}). Пробуємо Telethon...")
            
            # Якщо Bot API не впорався (наприклад, файл > 20МБ), пробуємо Telethon
...
async def download_file_telethon(bot, message_obj, file_name, temp_path):
    ...
    try:
        ...
        logger.info(f"Telethon завантаження: chat_id={chat_id}, message_id={message_id}, file_name={file_name}")
        
        # Отримуємо повідомлення через Telethon
        # Якщо це топік, Telethon може потребувати додаткових зусиль, але get_messages зазвичай працює
        ...
        if not telethon_message or not telethon_message.document:
            logger.error(f"Не вдалося знайти документ у повідомленні через Telethon ID {message_id}")
        ...
        logger.info(f"Файл {file_name} успішно завантажено через Telethon")
